...
const RASPBERRY_WHEP_NOIR_URL = `http://${RASPBERRY_IP}:8889/camara_noir/whep`;
const RASPBERRY_WHEP_RGB_URL = `http://${RASPBERRY_IP}:8889/camara_rgb/whep`;
const RASPBERRY_SNAPSHOT_URL = `http://${RASPBERRY_IP}:5000/capturar/ambas`;
    ...
    <View className="flex-1 relative bg-black justify-center gap-3">
      ...
      <View className="w-full aspect-video rounded-xl overflow-hidden">
        <View className="absolute top-2 left-2 z-10 flex-row items-center bg-black/50 px-3 py-1 rounded-full border border-white/15">
          <View className={`w-1.5 h-1.5 rounded-full mr-2 ${streamErrorNoir ? 'bg-red-500' : 'bg-green-500'}`} />
        ...
        {streamErrorNoir ? (
          ...
          <WebView
            ...
            <script>
              ...
              pc.createOffer().then(offer => {
                ...
                return fetch('${RASPBERRY_WHEP_NOIR_URL}', { method:'POST', headers:{'Content-Type':'application/sdp'}, body:offer.sdp });
          ...
          ` }}
            ...
            scrollEnabled={false}
            style={{ flex: 1 }}
            onMessage={(e) => { if (e.nativeEvent.data === 'error') setStreamErrorNoir(true); }}
      ...
      <View className="w-full aspect-video rounded-xl overflow-hidden">
        <View className="absolute top-2 left-2 z-10 flex-row items-center bg-black/50 px-3 py-1 rounded-full border border-white/15">
          <View className={`w-1.5 h-1.5 rounded-full mr-2 ${streamErrorRgb ? 'bg-red-500' : 'bg-green-500'}`} />
          <Text className="text-white text-xs font-semibold tracking-wide">Cámara RGB</Text>
        </View>
        ...
        {streamErrorRgb ? (
          ...
          <WebView
            ...
            <script>
              ...
              pc.createOffer().then(offer => {
                pc.setLocalDescription(offer);
                return fetch('${RASPBERRY_WHEP_RGB_URL}', { method:'POST', headers:{'Content-Type':'application/sdp'}, body:offer.sdp });
              }).then(r => r.text()).then(sdp => {
                pc.setRemoteDescription({ type:'answer', sdp });
              }).catch(() => { clearTimeout(timeout); window.ReactNativeWebView.postMessage('error'); });
            </script>
          ...
          ` }}
            ...
            scrollEnabled={false}
            style={{ flex: 1 }}
            onMessage={(e) => { if (e.nativeEvent.data === 'error') setStreamErrorRgb(true); }}
            onError={() => setStreamErrorRgb(true)}
            startInLoadingState={true}
            renderLoading={() => (
              <View style={styles.loaderContainer}>
                <ActivityIndicator size="large" color="#3b82f6" />
                <Text className="text-[#a3a3a3] mt-3 text-sm">Conectando a cámara RGB...</Text>
              </View>
            )}
          />
        )}
      ...
      {capturedPhoto && (
        <View className="absolute bottom-28 left-0 right-0 items-center">
          <PreviewImagen titulo="Captura NoIR" uri={capturedPhoto} onCancel={cancelarFoto} />
        </View>
      )}
      ...
      <View className="absolute bottom-4 left-0 right-0 flex-row justify-between items-center px-10 bg-black/40 py-2.5">
        ...
        <TouchableOpacity
          onPress={takePicture}
          disabled={isCapturing || streamErrorNoir}
          className={`w-20 h-20 rounded-full border-4 justify-center items-center ${isCapturing || streamErrorNoir ? 'border-white/20' : 'border-white/60'}`}
        >
          <View className={`w-16 h-16 rounded-full justify-center items-center ${isCapturing || streamErrorNoir ? 'bg-white/20' : 'bg-white/70'}`}>
